...
function setupEventListeners() {
    // Bouton ajouter formateur - CORRECTION ICI
    const addBtn = document.getElementById('addFormateur');
    if (addBtn) {
        addBtn.addEventListener('click', openAddModal);
    } else {
        console.error('Bouton "addFormateur" non trouvé');
        // Alternative: chercher par texte si l'ID n'existe pas
        const buttons = document.querySelectorAll('.btn-primary');
        buttons.forEach(btn => {
            if (btn.textContent.includes('Ajouter') || btn.textContent.includes('formateur')) {
                btn.id = 'addFormateur';
    ...
    joursCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            ...
            if (debutInput && finInput) {
                ...
                // Si le jour est décoché, vider les champs horaires
                if (!this.checked) {
                    debutInput.value = '';
                    finInput.value = '';
                } else {
                    // Si le jour est coché, mettre des valeurs par défaut
                    if (!debutInput.value) debutInput.value = '08:00';
                    if (!finInput.value) finInput.value = '17:00';
                }
            }
        });
...
// ==========================================
// GESTION DES MODALS - CORRECTION ICI
// ==========================================

function openAddModal() {
    console.log('Ouverture du modal d\'ajout'); // Debug
    currentFormateurId = null;
    
    // Mettre à jour les titres du modal
    ...
    const joursCheckboxes = document.querySelectorAll('.jour-header input[type="checkbox"]');
    joursCheckboxes.forEach(checkbox => {
        // Cocher seulement les jours de semaine par défaut
        if (checkbox.id !== 'samedi') {
            checkbox.checked = true;
        } else {
            checkbox.checked = false;
        }
        // Déclencher l'événement change pour mettre à jour les champs horaires
        checkbox.dispatchEvent(new Event('change'));
    });
    ...
    if (modal) {
        ...
        console.error('Modal "formateurModal" non trouvé');
    }
}
...
function openEditModal(id) {
    ...
    if (!formateur) {
        console.error('Formateur non trouvé avec ID:', id);
        return;
    }
    ...
    const specialitesCheckboxes = document.querySelectorAll('input[name="specialite"]');
    specialitesCheckboxes.forEach(checkbox => {
        checkbox.checked = formateur.specialites ? formateur.specialites.includes(checkbox.value) : false;
    });
    ...
    if (formateur.disponibilites) {
        Object.keys(formateur.disponibilites).forEach(jour => {
            ...
            if (checkbox) {
                ...
                // Déclencher l'événement change pour mettre à jour l'état des champs horaires
                checkbox.dispatchEvent(new Event('change'));
            }
            
            const debutInput = document.querySelector(`input[name="${jour}_debut"]`);
            const finInput = document.querySelector(`input[name="${jour}_fin"]`);
            if (debutInput && dispo.debut) debutInput.value = dispo.debut;
            if (finInput && dispo.fin) finInput.value = dispo.fin;
        });
    }
...
function openDeleteModal(id, nom) {
    ...
    if (deleteFormateurName) {
        deleteFormateurName.textContent = nom;
    }
...
// ==========================================
// GESTION DU FORMULAIRE - CORRECTION ICI
// ==========================================
...
function handleFormSubmit(e) {
    ...
    jours.forEach(jour => {
        const checkbox = document.getElementById(jour);
        const debut = formData.get(`${jour}_debut`);
        const fin = formData.get(`${jour}_fin`);
...
function deleteFormateur() {
    if (!currentFormateurId) {
        console.error('Aucun ID de formateur sélectionné pour la suppression');
        return;
    }
    ...
    if (index !== -1) {
        ...
        showToast('error', 'Erreur', 'Formateur non trouvé');
    }
}
            ...
            <tr>
                ...
                <td>
                    <div class="action-buttons">
                        ...
                        <button class="action-btn delete" onclick="openDeleteModal(${formateur.id}, '${formateur.nom.replace(/'/g, "\\'")}')" title="Supprimer">
...
// Rendre les fonctions accessibles globalement pour les appels depuis HTML
